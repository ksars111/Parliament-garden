        ...
        {!isExpanded && (
          ...
          >
            ...
            <div className="flex-1 min-w-0 h-full sm:h-auto flex flex-col sm:flex-row sm:items-center sm:justify-between py-1 sm:py-0 gap-3 sm:gap-4 pr-4 sm:pr-0">
              {/* Specimen details */}
              <div 
                className="flex-1 min-w-0 cursor-pointer text-left flex flex-col items-start justify-center gap-y-1" 
                onClick={(e) => {
                  e.stopPropagation();
                  if (canEdit) {
                    setIsExpanded(true);
              ...
              >
                <div className="flex flex-wrap items-center justify-start gap-x-2 gap-y-1">
                  <h3 className="font-extrabold text-lg sm:text-base md:text-lg text-zinc-900 leading-snug line-clamp-3 sm:line-clamp-2 break-words">{name || 'Unnamed Specimen'}</h3>
